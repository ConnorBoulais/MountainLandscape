...
void Shape::init_instance_buffer(std::vector<glm::vec4> &positions, const shared_ptr<Program2> prog)
{
	for (int w = 0; w < obj_count; w++)

	{
		// Initialize the vertex array object
		glBindVertexArray(vaoID[w]);
		int size = positions.size();
		instanceBufSize = size;
		glGenBuffers(1, &instanceBufID);
		glBindBuffer(GL_ARRAY_BUFFER, instanceBufID);
		glBufferData(GL_ARRAY_BUFFER, size * sizeof(glm::vec4), positions.data(), GL_STATIC_DRAW);
...
void Shape::draw(const shared_ptr<Program2> prog, bool use_extern_texures) const
{
	for (int i = 0; i < obj_count; i++)

	{
		...
		// Draw
		if (is_instanced) {
			glDrawElementsInstanced(GL_TRIANGLES, 6, GL_UNSIGNED_SHORT, (void*)0, instanceBufSize);
		}
		else {
			glDrawElements(GL_TRIANGLES, (int)eleBuf[i].size(), GL_UNSIGNED_INT, (const void*)0);
		}
